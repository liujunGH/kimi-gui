...
### 轮次 12 · kimi3 · 无证书 macOS 分发真相(实测)· 2026-07-24

- 实测(macOS 26):**完全无签名 + quarantine 也是直接 SIGKILL**(amfid 当场杀),报「无法打开」。b 方案(剥 adhoc)只是把「已损坏」换成「无法验证开发者」,体验都是死路。
- 无证书下收件人只有两条路:`xattr -dr com.apple.quarantine`(最快)或 双击一次失败后 系统设置→隐私与安全性→安全性→「仍要打开」。
- **结论:Apple 开发者证书 + 公证是无警告分发的唯一解**。当前 CI 的剥签名策略保留(文案稍友好),有证书后删掉该步。
- v1.0.6 全绿发行(Windows 托盘右键菜单 + daemon spawn 自启修复)。

### 轮次 12.1 · kimi3 · macOS 分发「已损坏」最终解 · 2026-07-24

- **实测(macOS 26)**:无签名=SIGKILL 必死(provenance 删不掉);**adhoc 签名能跑**;浏览器下载打 quarantine →「已损坏」,`xattr -dr com.apple.quarantine` 一次可解;**CLI 下载(curl/gh)不打 quarantine,直接开**。
- **v1.0.5 剥签名是方向性错误,已回滚(v1.0.7 起 adhoc 签名,DMG 实测)**。
