...
describe('Kimi Code contract version gate', () => {
  it('accepts 0.40+ on the v2 backend', () => {
    expect(supportsKimiContract('0.40.0', 'v2')).toBe(true);
    expect(supportsKimiContract('kimi 0.42.1', 'v2')).toBe(true);
  });

  it('rejects older versions and the legacy backend', () => {
    expect(supportsKimiContract('0.39.9', 'v2')).toBe(false);
    expect(supportsKimiContract('0.40.0', 'v1')).toBe(false);
